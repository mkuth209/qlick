...
// ✅ Field defined OUTSIDE App so it isn't recreated on every render
const Field = ({ id, label, placeholder, type="text", form, setForm, errors, setErrors }) => (
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;500;600;700;800&display=swap'); *{box-sizing:border-box;margin:0;padding:0} ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:${R};border-radius:2px} html{scroll-behavior:smooth} @keyframes fadeUp{from{opacity:0;transform:translateY(28px)}to{opacity:1;transform:translateY(0)}} @keyframes fadeIn{from{opacity:0}to{opacity:1}} @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}} @keyframes slideUp{from{opacity:0;transform:translateY(36px)}to{opacity:1;transform:translateY(0)}} @keyframes glow{0%,100%{opacity:0.3}50%{opacity:0.6}} .btn-r{background:${R};color:#fff;border:none;padding:12px 28px;border-radius:100px;font-size:14px;font-weight:700;cursor:pointer;transition:all 0.2s;font-family:inherit} .btn-r:hover{background:#ff4030;transform:translateY(-2px);box-shadow:0 12px 32px ${R}55} .btn-o{background:transparent;color:#fff;border:1px solid #333;padding:12px 28px;border-radius:100px;font-size:14px;font-weight:600;cursor:pointer;transition:all 0.2s;font-family:inherit} .btn-o:hover{border-color:#555;background:#1a1a1a} .fc{background:${CARD};border:1px solid ${BORDER};border-radius:20px;padding:24px;cursor:pointer;transition:all 0.3s} .fc:hover,.fc.active{border-color:${R}55;background:#161616;transform:translateY(-3px);box-shadow:0 8px 32px ${R}22} .sc{background:${CARD};border:1px solid ${BORDER};border-radius:20px;padding:28px;transition:all 0.3s} .sc:hover{border-color:${R}44;background:#161616} .fq{border-bottom:1px solid ${BORDER};padding:20px 0;cursor:pointer} .mo{position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:1000;display:flex;align-items:center;justify-content:center;padding:20px;animation:fadeIn 0.2s ease;backdrop-filter:blur(12px)} .mb{background:#111;border:1px solid #222;border-radius:28px;padding:32px;width:100%;max-width:480px;animation:slideUp 0.3s cubic-bezier(0.34,1.56,0.64,1);max-height:90vh;overflow-y:auto}`}</style>
      ...
      <div className="mb" dir={dir}>
        {!submitted ? (
          <>
            ...
            {step===1 ? (
              <>
                <Field id="fullName" label={ar?"الاسم الكامل":"Full Name"} placeholder={ar?"محمد أحمد":"Mohammed Ahmed"} form={form} setForm={setForm} errors={errors} setErrors={setErrors}/>
                <Field id="storeName" label={ar?"اسم المطعم":"Restaurant Name"} placeholder={ar?"مطعم البرجر الطازج":"Fresh Burger Restaurant"} form={form} setForm={setForm} errors={errors} setErrors={setErrors}/>
                <Field id="email" label={ar?"البريد الإلكتروني":"Email Address"} placeholder="example@email.com" type="email" form={form} setForm={setForm} errors={errors} setErrors={setErrors}/>
                <Field id="phone" label={ar?"رقم الجوال":"Phone Number"} placeholder="+966 5X XXX XXXX" type="tel" form={form} setForm={setForm} errors={errors} setErrors={setErrors}/>
